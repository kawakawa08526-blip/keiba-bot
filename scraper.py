...
def find_race_id(place: str, race_num: int) -> str:
    ...
    方法1: race_list.htmlのhrefからrace_idを直接抽出
    方法2: shutuba.html総当たり + RaceData02の「X日目」でnichi確認
    """
    ...
    for delta in [0, 1, 2]:
        ...
        try:
            ...
            if matched:
                ...
                return race_id
            print(f"[INFO] 方法1: {place}{race_num}R 見つからず(候補:{len(href_ids)}件)")
        except Exception as e:
            print(f"[ERROR] 方法1失敗: {e}")

        # 方法2: shutuba.html総当たり
        # RaceData02の「X日目」とnichi番号が一致するものを採用
        print(f"[INFO] 方法2: 総当たり {place}{race_num}R {date_str}")
        confirmed = []   # nichi_day == nichi で確認済み
        candidates = []  # 頭数OKだが日付未確認
        ...
        for kai in range(1, 4):
            for nichi in range(1, 10):
                ...
                try:
                    ...
                    # RaceData02から「X日目」を取得
                    d2 = soup.select_one(".RaceData02, [class*='RaceData02']")
                    if d2:
                        t2 = d2.get_text()
                        nichi_match = re.search(r'(\d+)日目', t2)
                        if nichi_match:
                            nichi_day = int(nichi_match.group(1))
                            print(f"[DEBUG] {race_id}: {nichi_day}日目 (nichi={nichi})")
                            if nichi_day == nichi:
                                confirmed.append((kai, nichi, race_id, len(horse_rows)))
                                print(f"[INFO] 方法2確認済み: {race_id} ({nichi_day}日目=nichi{nichi} OK)")
                                continue
                    candidates.append((kai, nichi, race_id, len(horse_rows)))
                except Exception:
                    continue

        # 確認済みを優先、なければ最大nichiの候補を使用
        pool = confirmed if confirmed else candidates
        if pool:
            best = max(pool, key=lambda x: (x[0], x[1]))
            print(f"[INFO] 方法2採用: {best[2]} (kai={best[0]},nichi={best[1]},{best[3]}頭)")
            return best[2]
...
def get_race_info(race_id: str) -> dict:
    ...
    try:
        ...
        d1 = None
        for sel in [".RaceData01", "[class*='RaceData01']"]:
        ...
        if d1:
            ...
            if m:
                info["distance"] = int(m.group(1))
            info["surface"] = "ダート" if "ダート" in page[:3000] else "芝"
        ...
        if d2:
            t2 = d2.get_text()
            print(f"[DEBUG] RaceData02: {t2[:80]}")
        else:
            t2 = res.text[:8000]
